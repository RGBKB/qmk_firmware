...
typedef struct {
	...
	// indicates that button has JUST timed out and needs to be handled)
...
typedef struct {
    ...
    uint8_t position; // Purely for special-key handling purposes (most users won't need this)
} half_touch_flags_t;
...
half_touch_status_t touch_half_state_j = { 0, 0, 0, 0, 0, 0, 0, 0 }; // Used on both halves (separately)
half_touch_flags_t touch_half_flags_j          = { 0, 0, 0, 0, 0, 0 }; // Used on both halves (separately)
half_touch_flags_t touch_master_prev_flags_j   = { 0, 0, 0, 0, 0, 0 }; // Unused on slave side
half_touch_flags_t touch_slave_prev_flags_j    = { 0, 0, 0, 0, 0, 0 }; // Unused on slave side
...
static void touch_encoder_update_position(uint8_t* position, uint8_t raw, uint8_t index) { // I've not refactored this at all!!!

    // If the user touches and moves enough, expire touch_timer faster and do encoder position logic instead
	...
	// Need to update the "most recent good/initial swipe position
	half_touch_state.position = ??? And need the raw logic
...
void touch_encoder_end_static_touch(uint8_t touch_handedness, half_touch_status_t *half_touch_state, half_touch_flags_t *half_touch_flags, uint8_t sectn) {
	...
	if (half_touch_state.holds & mask) { // Extracts current hold status for each section in turn. Any form of swiping and/or button timeout will toggle this off and handle key release
		...
		touch_encoder_released_kb(touch_handedness, sectn); // release whatever key was being pressed
	...
	if (half_touch_state.was_touching & mask) { // Extracts current hold status for each section in turn. Any form of swiping and/or button timeout will toggle this off and handle key release
		xprintf("tapped %d %d\n", touch_handedness, sectn);
		touch_encoder_tapped_kb(touch_handedness, sectn); // tap whatever key was being touched
...
void touch_encoder_start_end_touch(uint8_t touch_handedness, half_touch_status_t *half_touch_state, half_touch_flags_t *half_touch_flags) {
	// The early logic doesn't work for the slave side when called from the master half (after transport/memcpy) - only position and taps are updated then, not touch_processed!
...
//				}
		...
		if (delta & SLIDER_BIT) { // I.e. the touch is new (it's changed, and it's not ended (above logic --> return)
			...
			// Keydown the special key and send position to handler function in keymap.c
			touch_encoder_holding_kb(touch_handedness, -1);
			touch_encoder_raw_position(touch_raw[3]);
...
void touch_encoder_ongoing_touch(uint8_t touch_handedness, half_touch_status_t *half_touch_state,
	...
	if (touch_processed[0] & SLIDER_BIT) {
		...
		if (touch_processed[3] != touch_raw[3]) {
			// send the raw position to the handler function in keymap.c
			touch_encoder_raw_position(touch_raw[3]);
		...
		if (half_touch_state.swipes) {// Then user was swiping at last check, so must still be swiping now.
			...
			if (!touch_disabled) {
				touch_encoder_update_position(&touch_processed[3], touch_raw[3], touch_handness); // Still need to sort this function out
				return; // Then get out
			}
		}
		// else...
		...
		if (not_timed_out = 1) { // Then all recent (i.e. within one timeout period previously)
			...
			if ((uint8_t)(touch_raw[3] - half_touch_state.position) <= TOUCH_DEADZONE) return; // No need to do anything if true
			// else...
			// movement was more than the touch deadzone WITHIN one section, so a swipe must be sent
		...
		// else...
		// Then movement crossed an inter-section boundary, so some inference/user discretion is required
...
void touch_encoder_send_strokes(uint8_t touch_handedness, half_touch_flags_t *half_touch_flags){
...
void touch_encoder_update(int8_t transaction_id) {//This used to take void argument - changed for the transport, which is now 'less hacky'.
    ...
    if (is_keyboard_master()) { // If master...
    	touch_encoder_check_states(touch_handness, &touch_master_state_j, &touch_master_flags); // Not 100 % sure this should be a pointer tbh :/
    	touch_encoder_send_strokes(touch_handness, &touch_master_prev_flags_j, &touch_master_flags_j);
		...
		// I THINK this updates slave_state with the position and taps from the slave half
			if (memcmp(&touch_slave_flags_j, &slave_flags, sizeof(half_touch_flags_t))) {
			// I THINK this compares the current slave_state to the last time touch_slave_state was updated,
				if (!touch_slave_init) { // Not really sure if/why this is necessary. Shouldn't both
					...
					// is included?
					touch_slave_flags_j = slave_flags;
					touch_slave_init = true;
					return;
				}
				touch_encoder_send_strokes(!touch_handness, &touch_slave_flags_j, &slave_flags);
			}
		}
    }
    	...
    	touch_encoder_check_states(!touch_handness, &touch_slave_state_j, &touch_slave_flags_j);
...
void touch_encoder_slave_sync(uint8_t initiator2target_buffer_size, const void* initiator2target_buffer, uint8_t target2initiator_buffer_size, void* target2initiator_buffer) {
    ...
    memcpy(target2initiator_buffer, &touch_slave_flags_j, sizeof(half_touch_flags_t));
}

// Everything below this still needs refactoring and integrating into (above) the code above.
